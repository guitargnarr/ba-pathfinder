...
const RoadmapPage: React.FC<RoadmapPageProps> = ({ roadmap: initialRoadmap }) => {
  const { roadmap, setNodeStatus } = useRoadmapState(initialRoadmap);
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div>
            {roadmap.nodes.map((node) => (
              <RoadmapNode key={node.id} node={node} onStatusChange={setNodeStatus} />
            ))}
          </div>
        </div>
      </main>
